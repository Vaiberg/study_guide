...
`DISTINCT` позволяет выбрать только уникальные значения в указанном столбце.
...
- `EXISTS`
    ...
    Отличие от `IN`: `EXISTS` проверяет только факт существования, а не возвращает значения.
